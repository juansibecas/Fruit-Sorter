...
def run():
    ...
    path = 'E:\Escritorio\Facultad\Inteligencia Artificial I/frutas/'
    ...
    for fruit_index, fruit_name in enumerate(fruits): #data collection for stats
        ...
        for prop_index in range (rgb_length): #rgb mean by layer
            for image_index in range(training_images):   
    ...
    for image in testing_images:
        ...
        #knn predictions
        for knn_index in range(knn_ammount):
        ...
        #kmeans predictions
        for kmeans_index in range (kmeans_ammount):
...
def normalize_database(data): #normalizes the full database and also returns a max and min list for each component for further vector normalization
    vector_len = len(data[0]['vector']) #since all vectors are the same length, it doesn't matter which data index i choose
    data_len = len(data)
    max_and_min_list = []
    for vector_index in range(vector_len):
